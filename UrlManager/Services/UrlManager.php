...
class UrlManagerProcessor extends \RpcThrift\Services\RpcServiceBaseProcessor {
  ...
  protected function process_ParseRequest($seqid, $input, $output) {
    $args = new \UrlManager\Services\UrlManager_ParseRequest_args();
    $args->read($input);
    $input->readMessageEnd();
    $result = new \UrlManager\Services\UrlManager_ParseRequest_result();
    try {
      $result->success = $this->handler_->ParseRequest($args->request);
    } catch (\RpcThrift\Services\RpcException $re) {
      $result->re = $re;
    }
    if ($this->output_ instanceof TBinaryProtocol) {
      $bin_accel = ($output instanceof TBinaryProtocolAccelerated) && function_exists('thrift_protocol_write_binary');
      if ($bin_accel) {
        thrift_protocol_write_binary($output, 'ParseRequest', TMessageType::REPLY, $result, $seqid, $output->isStrictWrite());
      } else {
        $output->writeMessageBegin('ParseRequest', TMessageType::REPLY, $seqid);
        $result->write($output);
        $output->writeMessageEnd();
        $output->getTransport()->flush();
      }
    } else {
      sm_thrift_protocol_write_binary($output, 'ParseRequest', TMessageType::REPLY, $result, $seqid, $output->isStrictWrite());
    }
  }

  protected function process_CreateUrl($seqid, $input, $output) {
    $args = new \UrlManager\Services\UrlManager_CreateUrl_args();
    $args->read($input);
    $input->readMessageEnd();
    $result = new \UrlManager\Services\UrlManager_CreateUrl_result();
    try {
      $result->success = $this->handler_->CreateUrl($args->action, $args->params);
    } catch (\RpcThrift\Services\RpcException $re) {
      $result->re = $re;
    }
    if ($this->output_ instanceof TBinaryProtocol) {
      $bin_accel = ($output instanceof TBinaryProtocolAccelerated) && function_exists('thrift_protocol_write_binary');
      if ($bin_accel) {
        thrift_protocol_write_binary($output, 'CreateUrl', TMessageType::REPLY, $result, $seqid, $output->isStrictWrite());
      } else {
        $output->writeMessageBegin('CreateUrl', TMessageType::REPLY, $seqid);
        $result->write($output);
        $output->writeMessageEnd();
        $output->getTransport()->flush();
      }
    } else {
      sm_thrift_protocol_write_binary($output, 'CreateUrl', TMessageType::REPLY, $result, $seqid, $output->isStrictWrite());
    }
  }
}
